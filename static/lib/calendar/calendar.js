/*
 <input type="text" name="startdate" class="form-control" value="" style="width: 150px;" onclick="showcalendar(event, this)"   />
 <input type="text" name="enddate" class="form-control" value="" style="width: 150px;" onclick="showcalendar(event, this,true)"  />
 */
document.write('<div id="append_parent"></div><div id="ajaxwaitid"></div>');
var controlid = null;
var currdate = null;
var startdate = null;
var enddate = null;
...
function loadcalendar() {
    ...
    for (var k = 2020; k >= 1931; k--) {
        ...
        s += '<a href="javascript:;" onclick="refreshcalendar(' + k + ', mm);document.getElementById(\'calendar_year\').style.display=\'none\'"><span' + (today.getFullYear() == k ? ' class="calendar_today"' : '') + ' id="calendar_year_' + k + '">' + k + '</span></a><br />';
    }
    s += '</div></div>';
    s += '<div id="calendar_month" onclick="doane(event)" style="display: none;z-index:100001;">';
    for (var k = 1; k <= 12; k++) {
        s += '<a href="javascript:;" onclick="refreshcalendar(yy, ' + (k - 1) + ');document.getElementById(\'calendar_month\').style.display=\'none\'"><span' + (today.getMonth() + 1 == k ? ' class="calendar_today"' : '') + ' id="calendar_month_' + k + '">' + k + (k < 10 ? '&nbsp;' : '') + ' 月</span></a><br />';
    }
    s += '</div>';
...
function settime(d) {
    ...
    controlid.value = yy + "-" + zerofill(mm + 1) + "-" + zerofill(d) + (addtime ? ' ' + zerofill(document.getElementById('hour').value) + ':' + zerofill(document.getElementById((halfhour) ? 'minutehalfhourly' : 'minute').value) : '');
...
function confirmcalendar() {
    if (addtime && controlid.value === '') {
        controlid.value = today.getFullYear() + '-' + (today.getMonth() + 1) + '-' + today.getDate() + ' ' + zerofill(document.getElementById('hour').value) + ':' + zerofill(document.getElementById((halfhour) ? 'minutehalfhourly' : 'minute').value);
    }
    closecalendar();
}
...
function showdiv(id) {
    var p = fetchOffset(document.getElementById(id));
